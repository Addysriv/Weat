...
public class RegistrationV5ControllerImpl implements RegistrationV5Controller {
	...
	@Override
	public ResponseEntity<ApiResponse> initiatePhoneLogin(@Valid PhoneNumberDto phoneNumberDto,HttpServletRequest request) {
		...
		return CoreUtil.buildApiResponse(SignupHelper.toDtoMasked(registration), request, messages.get("user.notcreated"), HttpStatus.CREATED.value(),
				HttpStatus.NOT_FOUND.value(), HttpStatus.CREATED, HttpStatus.NOT_FOUND);
	}
	...
	public ResponseEntity<ApiResponse> completeSignup(@Valid SignUpRequestV5 signUpRequestV4,
		...
		User user = registrationService.signup(signUpRequestV4, headers, clientIp);
		return CoreUtil.buildApiResponse(user, request, messages.get("user.notcreated"), HttpStatus.CREATED.value(),
				HttpStatus.NOT_FOUND.value(), HttpStatus.CREATED, HttpStatus.NOT_FOUND);
	}
	...
	public ResponseEntity<ApiResponse> validateOtp(String regId, OtpDto otp, HttpServletRequest request) {
		...
		if(registration.getRegistrationStatus() ==  RegistrationStatus.EXISTING_USER) {
			String clientIp = SignupHelper.getClientIpAddress(request);
			User user = userService.getUser(registrationDto.getUserId());
